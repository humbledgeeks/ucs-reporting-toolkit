function Invoke-AsBuiltReport.Cisco.UcsManager {
    ...
    .NOTES
        ...
        Twitter:        @tpcarman
        Github:         tpcarman
        Modernized:     e360 / HumbledGeeks (v0.3.0, PowerShell 7 / Core)
    ...
    # Import JSON Configuration for Section, InfoLevel and HealthCheck
    $InfoLevel = $ReportConfig.InfoLevel
    $Section = $ReportConfig.Section
    $HealthCheck = $ReportConfig.HealthCheck
    
    #region Configuration Settings
    # UCS Fabric Interconnects present a self-signed certificate. On PowerShell 7 /
    # Core, Connect-Ucs fails certificate validation by default, so relax it for the
    # UCS PowerTool session and allow multiple simultaneous default connections.
    # (Report styling is applied automatically by AsBuiltReport.Core from this
    # module's .Style.ps1, or overridden with New-AsBuiltReport -StyleFilePath.)
    try {
        $null = Set-UcsPowerToolConfiguration -InvalidCertificateAction Ignore -Confirm:$false -ErrorAction SilentlyContinue
        $null = Set-UcsPowerToolConfiguration -SupportMultipleDefaultUcs $true -Confirm:$false -ErrorAction SilentlyContinue
    } catch {
        Write-PScriboMessage -IsWarning "Unable to set UCS PowerTool configuration: $($_.Exception.Message)"
    }

    foreach ($UCS in $Target) {
        # Clear any pre-existing default UCS session(s) first, so unscoped queries
        # cannot return duplicate objects from more than one default connection
        # (e.g. a leftover interactive Connect-Ucs session left open before the run).
        if ($DefaultUcs) { $null = Disconnect-Ucs -Ucs $DefaultUcs -ErrorAction SilentlyContinue }
            ...
            Section -Style Heading1 -Name "$($UcsSystem.Name)" {
                ...
                if ($Section.Servers) {
                    Section -Style Heading2 -Name 'Servers' {
                        ...
                        $UcsServiceProfiles = Get-UcsServiceProfile -Ucs $UCSM | Where-Object { $_.Type -eq 'instance' } | Sort-Object Dn -Unique | Sort-Object Name
                        ...
                        $UcsServiceProfileTemplates = Get-UcsServiceProfile -Ucs $UCSM | Where-Object { $_.Type -ne 'instance' } | Sort-Object Dn -Unique | Sort-Object Name
